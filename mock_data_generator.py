...
# Google Gemini Setup for AI response
client = genai.Client(api_key="your_api_key")  # Provide your actual API key
client.models.gemini("2.1-flash", contents="your-prompt")
...
if mode == "AI Chatbot Mode":
    ...
    if prompt:
        ...
        schema_info = response.text.strip()

        # Assuming the AI response contains table names, columns, and data types
        # Extract and store schema information
        st.session_state.schema = parse_schema_response(schema_info)  # You should create parse_schema_response() to extract data
        st.session_state.schema_updated = True
        st.session_state.messages.append({"role": "assistant", "content": schema_info})
...
# Populate form fields with the AI-generated schema
if st.session_state.schema_updated:
    # Show the tables and columns that were generated by AI
    for table_name, table_info in st.session_state.schema['tables'].items():
        ...
        st.session_state.schema[table_name]["columns"] = columns.split(", ")
        st.session_state.schema[table_name]["data_types"] = data_types.split(", ")
...
# Function to parse schema response (to be defined based on your AI response format)
def parse_schema_response(response):
    # Dummy schema parsing (replace with actual AI response parsing)
    schema = {
        "tables": {
            "Product": {
